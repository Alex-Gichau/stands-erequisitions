...
export function ContactFinanceModal({ isOpen, onClose, currentUser }: ContactFinanceModalProps) {
  ...
  return (
    <div id="contact-finance-modal" className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/65 dark:bg-slate-950/80 backdrop-blur-sm">
      <motion.div
      ...
      >
        ...
        <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-black text-slate-900 dark:text-white tracking-tight uppercase">Contact Finance Office</h3>
          ...
          <AnimatePresence mode="wait">
            {success ? (
              ...
              >
                ...
                <div>
                  <h4 className="text-lg font-black text-slate-900 dark:text-white tracking-tight uppercase">Opening Email Client</h4>
                ...
                <div className="flex flex-col sm:flex-row gap-3 w-full max-w-xs justify-center pt-2">
                  ...
                  <button
                    type="button"
                    onClick={onClose}
                    className="flex-1 px-5 py-2.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-xl text-xs font-black uppercase tracking-wider transition-all cursor-pointer"
                  >
                    Done
                  </button>
                </div>
              ...
              >
                ...
                <div className="p-4 bg-slate-50 dark:bg-slate-950/60 border border-slate-100 dark:border-slate-800 rounded-2xl flex items-center justify-between text-xs font-bold text-slate-600 dark:text-slate-300">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-slate-800 dark:text-white">{currentUser.name} ({currentUser.email})</p>
                  </div>
                  <div className="text-right space-y-0.5">
                    <p className="text-[9px] text-slate-400 uppercase tracking-widest">Recipient</p>
                    <p className="text-emerald-700 dark:text-emerald-400 font-extrabold uppercase">Finance Office</p>
                  </div>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 focus:border-emerald-500 focus:bg-white dark:focus:bg-slate-900 rounded-2xl px-4 py-3 text-xs font-bold text-slate-800 dark:text-slate-100 focus:outline-none transition-all placeholder:text-slate-400 dark:placeholder:text-slate-600"
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 focus:border-emerald-500 focus:bg-white dark:focus:bg-slate-900 rounded-2xl px-4 py-3 text-xs font-bold text-slate-800 dark:text-slate-100 focus:outline-none transition-all placeholder:text-slate-400 dark:placeholder:text-slate-600 font-sans leading-relaxed resize-none"
                  />
                </div>

                {/* Info Note */}
                <p className="text-[10px] text-slate-400 leading-normal font-bold uppercase tracking-wider">
                  💡 Note: Sending will launch your local device mail client pre-addressed to <span className="text-emerald-600 dark:text-emerald-400 font-extrabold font-mono text-xs">finance@pceastandrews.org</span>.
                </p>
                ...
                <div className="flex flex-col sm:flex-row gap-3 pt-2">
                  <button
                    type="button"
                    onClick={onClose}
                    className="flex-1 py-3 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all cursor-pointer text-center"
                  >
                    Cancel
                  </button>
                  <button
