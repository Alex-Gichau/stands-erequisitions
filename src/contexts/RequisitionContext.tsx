...
export function safeNormalizeComments(comments: any): any[] {
  ...
  return parsed.map((c: any) => {
    ...
    // Normalize nested replies recursively if present
    const rawReplies = Array.isArray(c.replies) ? c.replies : [];
    const normalizedReplies = rawReplies.map((r: any) => {
      if (!r || typeof r !== 'object') return null;
      const rAvatar = r.authorAvatar || r.author_avatar || r.authorPhotoURL || r.author_photo_url || "";
      ...
      return {
        ...
        authorName: rName,
        authorEmail: rEmail,
        authorRole: r.authorRole || r.author_role || "USER",
        authorAvatar: rAvatar,
        authorPhotoURL: rAvatar,
        text: r.text || "",
  ...
  const loginWithEmail = async (email: string, pass: string) => {
    try {
      ...
      if (checkRes && checkRes.ok) {
        ...
        if (checkData.preRegistered && checkData.tempPasswordMatched) {
          try {
            ...
            if (userCredential.user) {
              ...
              }).catch(e => console.warn("Failed link profile background:", e));
              
              dispatchLoginSlackAlert(email, "Pre-Registered Password Activation", userCredential.user.uid).catch(() => {});
              return;
            }
          } catch (signUpErr: any) {
            if (signUpErr.code === "auth/email-already-in-use") {
              ...
              if (userCredential.user) {
                ...
                dispatchLoginSlackAlert(email, "Email & Password", userCredential.user.uid).catch(() => {});
                return;
              }
            }
            throw signUpErr;
